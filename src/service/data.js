export const data = {
    subtile: 'Junte-se a nós',
    title: '+ de 100 alunos que confiam  nós',
    prev: 'Cada dia traz consigo um novo conjunto de possibilidades de aprendizado.',
    cards: [
        {
            title: 'Formação Academica',
            description: '...'
        },
        {
            title: 'Cursos extracurriculares',
            description: '...'
        },
        {
            title: 'Pedagogia',
            description: '...'
        }
    ],
    about: `O Centro Educacional Pinheiros nasceu com o comprometimento e dedicação em proporcionar uma educação de qualidade e transparência desde à Educação Infantil até o Ensino Médio.
        Somos uma escola viva, desde 1993 desenvolvemos um trabalho de excelência. Com o compromisso de oferecer um ensino inovador alicerçado no rigor conceitual. Com mais de 30 anos, acreditamos que a beleza da vida vai além de uma sala de aula.
        Acreditamos que a transparência seja de vital importância no relacionamento entre responsáveis/alunos e a escola.`,
    couses: [
        {
            image: './playground.png',
            title: 'Educação infantil',
            description: 'Nossa proposta pedagógica é formulada considerando a criança como um ser em contrução, mas que já possui inúmeros conhecimentos e estes devem ser ampliados e articulados a uma realidade concreta. Neste sentido, devem ser trabalhados temas que envolvam e auxiliem:',
            cards: [
                {
                    title: 'Gestão de emoções'
                },
                {
                    title: 'Hábitos higiênico'
                },
                {
                    title: 'Preservação da natureza'
                }
            ],
            working: 'Deve-se entender que, para as crianças de 0 (zero) a 5 (cinco) anos, independentemente das diferentes condições no espaço escolar, as relações sociais requerem a atenção intensiva dos profissionais da educação, durante o tempo e o momento de desenvolvimento das atividades que lhes são peculiares: este é o tempo em que a curiosidade deve ser estimulada, a partir da brincadeira orientada pelos profissionais da educação.',
            objectives: `
                A Educação Infantil, cuja finalidade primordial é o desenvolvimento integral da 
                criança até cinco anos de idade, nos aspectos físico, psicológico, intelectual e 
                social, complementando a ação da família e da comunidade, tem como objetivos:
                    n/    
                        a) Estabelecer vínculos afetivos, ampliando suas possibilidades de comunicação e interação social;
                        n/
                        b) Respeitar a diversidade e desenvolver atitudes de ajuda e colaboração;
                        n/
                        c) Incentivar a autonomia nas atividades cotidianas, assim como nas descobertas e aprendizagens;
                        n/
                        d) Observar e explorar o ambiente com atitude de curiosidade;
                        n/
                        f) Brincar, expressando emoções, sentimentos, pensamentos, desejos e necessidades;
                        n/
                        g) Utilizar as diferentes linguagens de forma a compreender e ser compreendido, expressar suas idéias, sentimentos, necessidades e desejos;
                        n/
                        h) Conhecer algumas manifestações culturais, de interesse, respeito e participação, valorizando a diversidade.
                    n/
                Visamos formar cidadãos ativos, críticos e conscientes sobre si e sobre a realidade que os cerca, 
                além de incentivar o desenvolvimento humano em sentido pleno. A aplicabilidade desta proposta se faz 
                extremamente necessária desde as mais iniciais fases da educação e deve ser mantida até as fases finais, 
                criando um conjunto coerente e focado num único objetivo: formação de seres humanos que transformem o mundo.
            `
        },
        {
            image: './numbers.png',
            title: 'Jardim I',
            description: 'Nossa proposta pedagógica é formulada considerando a criança como um ser em contrução, mas que já possui inúmeros conhecimentos e estes devem ser ampliados e articulados a uma realidade concreta. Neste sentido, devem ser trabalhados temas que envolvam e auxiliem:',
            cards: [
                {
                    title: 'Gestão de emoções'
                },
                {
                    title: 'Hábitos higiênico'
                },
                {
                    title: 'Preservação da natureza'
                }
            ],
            working: 'Deve-se entender que, para as crianças de 0 (zero) a 5 (cinco) anos, independentemente das diferentes condições no espaço escolar, as relações sociais requerem a atenção intensiva dos profissionais da educação, durante o tempo e o momento de desenvolvimento das atividades que lhes são peculiares: este é o tempo em que a curiosidade deve ser estimulada, a partir da brincadeira orientada pelos profissionais da educação.',
            objectives: `
                A Educação Infantil, cuja finalidade primordial é o desenvolvimento integral da 
                criança até cinco anos de idade, nos aspectos físico, psicológico, intelectual e 
                social, complementando a ação da família e da comunidade, tem como objetivos:
                    n/    
                        a) Estabelecer vínculos afetivos, ampliando suas possibilidades de comunicação e interação social;
                        n/
                        b) Respeitar a diversidade e desenvolver atitudes de ajuda e colaboração;
                        n/
                        c) Incentivar a autonomia nas atividades cotidianas, assim como nas descobertas e aprendizagens;
                        n/
                        d) Observar e explorar o ambiente com atitude de curiosidade;
                        n/
                        f) Brincar, expressando emoções, sentimentos, pensamentos, desejos e necessidades;
                        n/
                        g) Utilizar as diferentes linguagens de forma a compreender e ser compreendido, expressar suas idéias, sentimentos, necessidades e desejos;
                        n/
                        h) Conhecer algumas manifestações culturais, de interesse, respeito e participação, valorizando a diversidade.
                    n/
                Visamos formar cidadãos ativos, críticos e conscientes sobre si e sobre a realidade que os cerca, 
                além de incentivar o desenvolvimento humano em sentido pleno. A aplicabilidade desta proposta se faz 
                extremamente necessária desde as mais iniciais fases da educação e deve ser mantida até as fases finais, 
                criando um conjunto coerente e focado num único objetivo: formação de seres humanos que transformem o mundo.
            `
        },
        {
            image: './logical-thinking.png',
            title: 'Jardim II',
            description: 'Nossa proposta pedagógica é formulada considerando a criança como um ser em contrução, mas que já possui inúmeros conhecimentos e estes devem ser ampliados e articulados a uma realidade concreta. Neste sentido, devem ser trabalhados temas que envolvam e auxiliem:',
            cards: [
                {
                    title: 'Gestão de emoções'
                },
                {
                    title: 'Hábitos higiênico'
                },
                {
                    title: 'Preservação da natureza'
                }
            ],
            working: 'Deve-se entender que, para as crianças de 0 (zero) a 5 (cinco) anos, independentemente das diferentes condições no espaço escolar, as relações sociais requerem a atenção intensiva dos profissionais da educação, durante o tempo e o momento de desenvolvimento das atividades que lhes são peculiares: este é o tempo em que a curiosidade deve ser estimulada, a partir da brincadeira orientada pelos profissionais da educação.',
            objectives: `
                A Educação Infantil, cuja finalidade primordial é o desenvolvimento integral da 
                criança até cinco anos de idade, nos aspectos físico, psicológico, intelectual e 
                social, complementando a ação da família e da comunidade, tem como objetivos:
                    n/    
                        a) Estabelecer vínculos afetivos, ampliando suas possibilidades de comunicação e interação social;
                        n/
                        b) Respeitar a diversidade e desenvolver atitudes de ajuda e colaboração;
                        n/
                        c) Incentivar a autonomia nas atividades cotidianas, assim como nas descobertas e aprendizagens;
                        n/
                        d) Observar e explorar o ambiente com atitude de curiosidade;
                        n/
                        f) Brincar, expressando emoções, sentimentos, pensamentos, desejos e necessidades;
                        n/
                        g) Utilizar as diferentes linguagens de forma a compreender e ser compreendido, expressar suas idéias, sentimentos, necessidades e desejos;
                        n/
                        h) Conhecer algumas manifestações culturais, de interesse, respeito e participação, valorizando a diversidade.
                    n/
                Visamos formar cidadãos ativos, críticos e conscientes sobre si e sobre a realidade que os cerca, 
                além de incentivar o desenvolvimento humano em sentido pleno. A aplicabilidade desta proposta se faz 
                extremamente necessária desde as mais iniciais fases da educação e deve ser mantida até as fases finais, 
                criando um conjunto coerente e focado num único objetivo: formação de seres humanos que transformem o mundo.
            `
        }
        
    ],

    links: {
        facebook: 'CEMASTRJ',
        instagram: 'centroeducacionalmast',
        whatsapp: '5521974987702',
        email: 'example@email.com',
        phone: '(21) 96931-4810',
        adress: 'Estr. Rio São Paulo Variante - Jardim Guandu, Nova Iguaçu - RJ, 26297-459'
    }
};